...
const extractConsultations = (payload: unknown): IConsultation[] => {
  if (Array.isArray(payload)) return payload as IConsultation[];
  ...
  for (const pool of pools) {
    if (Array.isArray(pool)) return pool as IConsultation[];
...
export default function ConsultationsList({
  ...
  const payNowMutation = useMutation({
    mutationFn: (consultationId: string) =>
      initiateConsultationPayment(consultationId),
    onSuccess: (result) => {
      toast.success("Redirecting to secure checkout...", {
        description: "Complete payment to confirm your consultation.",
      });
  ...
  // -------------------------
  const bookingsWithRedirectState = useMemo(() => {
    if (redirectStatus !== "success") return bookings;

    return bookings.map((booking) => {
      const matchesByConsultationId =
        highlightedConsultationId && booking.id === highlightedConsultationId;

      const matchesByPaymentId =
        redirectPaymentId &&
        booking.payment?.id &&
        booking.payment.id === redirectPaymentId;

      if (!matchesByConsultationId && !matchesByPaymentId) return booking;

      return { ...booking, paymentStatus: "PAID" as const };
    });
  }, [bookings, highlightedConsultationId, redirectPaymentId, redirectStatus]);
  ...
  useEffect(() => {
    if (redirectStatus !== "cancelled" || cancelToastShownRef.current) return;

    toast.warning("Payment cancelled", {
      description: "Your consultation payment was cancelled. You can review the booking below and pay again anytime.",
    });
  ...
  useEffect(() => {
    if (redirectStatus !== "success" || successToastShownRef.current) return;
    ...
    const intervalId = window.setInterval(() => void refetch(), 4000);
    const timeoutId = window.setTimeout(() => window.clearInterval(intervalId), 20000);
  ...
  useEffect(() => {
    if (!highlightedConsultationId || !bookingsWithRedirectState.length) return;
