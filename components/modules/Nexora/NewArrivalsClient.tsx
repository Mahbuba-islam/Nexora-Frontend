...
export default function NewArrivalsClient() {
  const { data: flagged = { data: [] }, isLoading: loadingFlagged } = useQuery({
    queryKey: ["products", { isNewArrival: true, limit: 24 }],
    queryFn: () => getProducts({ isNewArrival: true, limit: 24 }),
    staleTime: 1000 * 30,
  });
  const { data: freshest = { data: [], meta: {} }, isLoading: loadingFreshest } = useQuery({
    queryKey: ["products", { sortBy: "createdAt", sortOrder: "desc", limit: 24 }],
    queryFn: () => getProducts({ sortBy: "createdAt", sortOrder: "desc", limit: 24 }),
    staleTime: 1000 * 30,
    enabled: flagged.data.length < 6,
  });
  const products = flagged.data.length >= 6 ? flagged.data : freshest.data;
  const hero = products[0];
  const restRow = products.slice(1, 4);
  const grid = products.slice(4);
  const total = (flagged.meta?.total ?? freshest.meta?.total) ?? products.length;

  if (loadingFlagged || (flagged.data.length < 6 && loadingFreshest)) {
    ...
    <div className="bg-background">
      ...
      {products.length === 0 ? (
        ...
        <div className="mx-auto max-w-7xl px-4 py-14 md:px-8 md:py-20">
          ...
          {/* Product grid */}
          <section className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
            {grid.map((p) => (
